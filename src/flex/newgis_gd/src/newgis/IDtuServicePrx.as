package newgis{
	...
	public class IDtuServicePrx extends RpcProxyBase{
		//# -- INTERFACE PROXY -- 
		public var delta:Object = null;
		
		public function  IDtuServicePrx(conn:RpcConnection){
			this.conn = conn;
		}		
		public static function create(host:String,port:int):IDtuServicePrx{
			var conn:RpcConnection = new RpcConnection();
			conn.open(host,port);
			var prx:IDtuServicePrx = new IDtuServicePrx(conn);
			return prx;
		}		
		public static function createWithProxy(proxy:RpcProxyBase):IDtuServicePrx{
			var prx:IDtuServicePrx = new IDtuServicePrx(proxy.conn);
			return prx;
		}		
