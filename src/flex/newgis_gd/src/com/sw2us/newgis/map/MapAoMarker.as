...
package com.sw2us.newgis.map
{
	...
	public class MapAoMarker
	{
		...
		private var _marker:MMarker = null; 
		
		public function MapAoMarker(mao:MapAo,marker:MMarker)
		{
//			super(null,null,null);
		...
		public function get marker():MMarker{
			return _marker;
		}
		...
		public function set visible(show:Boolean):void{
		...
		*/
		public static function create(mao:MapAo,									  
			...
			var labelOptions:MLabelOptions=new MLabelOptions();  //新建文字标签参数选项对象 			
			var fontstyle:MFontStyle = new MFontStyle();  //定义字体风格对象 			
			fontstyle.name="Arial"  //定义字体 			
			fontstyle.size=12;     //定义字号 			
			fontstyle.color= 0xff0000;  //定义字体颜色 			
			fontstyle.bold=false;   //定义是否粗体 			
			labelOptions.fontStyle= fontstyle;     //设置文字标签中的文字风格 
			labelOptions.alpha=0.7;   //设置文字标签透明度 
			//labelOptions.zoomLevels=[10,11,12];  //设置文字标签在10、11、12级别下显示 
			
			labelOptions.content= mao.getActiveObject().getInfo().name;   //设置文字标签的显示内容 			
			labelOptions.hasBackground=true;   //设置文字标签是否背景 			
			labelOptions.hasBorder=true;     //设置文字标签是否有边框 			
			labelOptions.borderColor=0x0000ff;   //设置文字标签的背景颜色 			
			labelOptions.backgroundColor=0xffff00;   //设置文字标签的边框颜色 
			
			var markerOption:MMarkerOptions=new MMarkerOptions(); 			
			markerOption.imageUrl=imgurl; 			
			markerOption.picAgent=false; 			
			markerOption.imageAlign=MAlignPosition.MIDDLE_CENTER; 	
			markerOption.labelOption = labelOptions;
			markerOption.labelAlign = MAlignPosition.BOTTOM_CENTER;
				
			var marker:MMarker=new MMarker(new MLngLat(point.lon,point.lat),markerOption); 			
			marker.id=mao.getActiveObject().getId().toString(); 
			
			var map:MFlexMap;
			map = mao.layer.map.getMap() as MFlexMap;
			map.addOverlay(marker,true);			
			
			var r:MapAoMarker = new MapAoMarker(mao,marker); 
			return r;
		}
		...
		public function getBounds():Rectangle{
			...
			cont = _marker.image.parent.parent;
			
			//trace("bound0:",cont.getBounds( map) );
			r = cont.getBounds( map);
			/*
			cont = cont.stage;
			if(cont)
			trace("bound1:",cont.toString(),cont.x,cont.y,cont.width,cont.height);
			
			cont = cont.stage;
			if(cont)
			trace("bound2:",cont.toString(),cont.x,cont.y,cont.width,cont.height);
			cont = cont.stage;
			if(cont)
			trace("bound3:",cont.toString(),cont.x,cont.y,cont.width,cont.height);
			*/
