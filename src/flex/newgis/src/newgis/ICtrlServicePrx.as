package newgis{
	...
	public class ICtrlServicePrx extends RpcProxyBase{
		//# -- INTERFACE PROXY -- 
		public var delta:Object = null;
		
		public function  ICtrlServicePrx(conn:RpcConnection){
			this.conn = conn;
		}		
		public static function create(host:String,port:int):ICtrlServicePrx{
			var conn:RpcConnection = new RpcConnection();
			conn.open(host,port);
			var prx:ICtrlServicePrx = new ICtrlServicePrx(conn);
			return prx;
		}		
		public static function createWithProxy(proxy:RpcProxyBase):ICtrlServicePrx{
			var prx:ICtrlServicePrx = new ICtrlServicePrx(proxy.conn);
			return prx;
		}		
